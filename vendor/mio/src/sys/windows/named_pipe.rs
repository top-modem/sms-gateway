...
fn connect_done(status: &OVERLAPPED_ENTRY, events: Option<&mut Vec<Event>>) {
    ...
    // NOTE(sms-gateway patch): upstream `debug_assert_eq!(status.bytes_transferred(), 0)`
    // removed — a cancelled connect may legitimately report partial bytes.
    unsafe {
...
fn read_done(status: &OVERLAPPED_ENTRY, events: Option<&mut Vec<Event>>) {
    ...
    unsafe {
        match me.result(status.overlapped()) {
            ...
            Err(e) => {
                // NOTE(sms-gateway patch): upstream
                // `debug_assert_eq!(status.bytes_transferred(), 0)` removed — a
                // cancelled serial read (dropped SerialStream) completes with an
                // error AND partial bytes; the error state is still correct.
                io.read = State::Err(e);
            }
        }
    }
    ...
    unsafe {
        match me.result(status.overlapped()) {
            ...
            Err(e) => {
                // NOTE(sms-gateway patch): upstream
                // `debug_assert_eq!(status.bytes_transferred(), 0)` removed — a
                // cancelled serial write may complete with an error AND partial bytes.
                io.write = State::Err(e);
                io.notify_writable(&me, events);
            }
        }
